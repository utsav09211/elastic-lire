...
public class ImageHashLimitQuery extends Query {
			...
			if (scorer != null) {
				...
				if (newDoc == doc) {
					...
					if (getBoost() != 1.0f) {
						...
						return Explanation.match(score, "ImageHashLimitQuery, product of:",
								Explanation.match(getBoost(), "boost"));
					}

					return Explanation.match(score, "ImageHashLimitQuery, product of:",
							Explanation.match(score, "image score (1/distance)"));
	...
	public Weight createWeight(IndexSearcher searcher, boolean needsScore) throws IOException {
		...
		if (topDocs.scoreDocs.length == 0) { // no result find
			return Queries.newMatchNoDocsQuery().createWeight(searcher, needsScore);
		}
